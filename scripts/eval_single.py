...
parser.add_argument("--img_root", type=str, default="/root/audio-visual-gaze/test/test_frames")
parser.add_argument("--json_path", type=str, default="/root/audio-visual-gaze/test/test_frames/test_preprocessed.json")
parser.add_argument("--model_name", type=str, default="gazelle_dinov2_vitb14")
parser.add_argument("--ckpt_path", type=str, default="/root/gazelle_dinov2_vitb14.pt")
...
def parse_vid_frame(name: str):
    base = name.split('.')[0]
    video_id = base[:3]
    frame_num = int(base[3:6]) if len(base) >= 6 else 0
...
class VGSDataset(torch.utils.data.Dataset):
    def __init__(self, json_path, img_root, transform, audio_wav_root=None, audio_fps=30, audio_win_sec=0.5):
        ...
        for item in data:
            rel_path = item.get("path") or item.get("fname") or item.get("file_name")
            if rel_path is None:
                continue
            img_path = os.path.join(img_root, rel_path)
            if not os.path.isfile(img_path):
               
                candidate = os.path.join(img_root, os.path.basename(rel_path))
            ...
            if isinstance(heads, list) and len(heads) > 0:
                frame_name = os.path.basename(img_path)
                vid, frame_idx = parse_vid_frame(frame_name)
                for h in heads:
                    inout = int(h.get("inout", 1))
                    if inout != 1:
                        continue
                    bbox = h.get("bbox") or h.get("bbox_px")
                    if bbox is None:
                        continue
                    gx_list = h.get("gazex") or h.get("gaze_x")
                    gy_list = h.get("gazey") or h.get("gaze_y")
                    if gx_list is None or gy_list is None:
                        continue
                    gx = gx_list[0] if isinstance(gx_list, list) else gx_list
                    gy = gy_list[0] if isinstance(gy_list, list) else gy_list
                    self.items.append((img_path, [bbox[0], bbox[1], bbox[2], bbox[3]], [gx, gy], vid, frame_idx))
                continue
