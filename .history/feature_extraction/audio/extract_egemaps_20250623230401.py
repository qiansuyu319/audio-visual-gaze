...
def sliding_window_extract(input_wav, output_dir, window_size=1.0, stride=0.04, sr=16000):
    ...
    # If multi-channel, convert to mono by taking the first channel
    if data.ndim == 2:
        print(f"Input audio has {data.shape[1]} channels, using the first channel only.")
        data = data[:, 0]
    if file_sr != sr:
    ...
    for i in range(n_windows):
        ...
        if len(chunk) < win_len:
            # Skip last chunk if it's shorter than window_size
            continue
