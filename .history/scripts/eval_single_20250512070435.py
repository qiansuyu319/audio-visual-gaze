...
def evaluate_single_image(image_path, bbox_px, gaze_px, model_name, ckpt_path, save_path=None):
    ...
    # === 归一化 heatmap，使最大值为红色 ===
    normalized_heatmap = (heatmap_resized - np.min(heatmap_resized)) / (np.max(heatmap_resized) - np.min(heatmap_resized) + 1e-8)
    ...
    # 仅叠加高亮区域（值大的区域）
    mask = (normalized_heatmap > 0.2).astype(np.uint8)[:, :, np.newaxis]
    vis_image = np.where(mask, cv2.addWeighted(original_image, 0.6, heatmap_color, 0.4, 0), original_image)
    ...
    plt.axis('off')
    plt.title("Gaze Prediction with Local Heatmap and Line")
    plt.show()
