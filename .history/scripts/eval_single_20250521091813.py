...
def evaluate_dataset(
    ...
    all_records = []
    # 遍历 JSON 文件而非子文件夹
    for anno_filename in sorted(os.listdir(json_root_folder)):
        if not anno_filename.endswith(json_suffix):
            continue
        video_id = anno_filename[:-len(json_suffix)]
        anno_path = os.path.join(json_root_folder, anno_filename)

        # 对应的图片文件夹
        video_folder = os.path.join(img_root_folder, video_id)
        if not os.path.isdir(video_folder):
            print(f"[WARN] Missing image folder for {video_id}, skipped.")
            continue
