...
def evaluate_dataset(
    ...
    for video_id in sorted(os.listdir(img_root_folder)):
        ...
        # JSON 文件在同一文件夹下，命名如 002_GT_VideoFormat.json
        anno_filename = f"{video_id}{json_suffix}"
        anno_path = os.path.join(video_folder, anno_filename)
        if not os.path.isfile(anno_path):
            print(f"[WARN] Missing annotation file {anno_filename} in {video_folder}, skipped.")
            continue
