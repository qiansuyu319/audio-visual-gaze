...
def gazefollow_auc(heatmap, gt_gazex, gt_gazey, height, width):
    target_map = np.zeros((height, width), dtype=np.uint8)

    for point in zip(gt_gazex, gt_gazey):
        if point[0] >= 0:
            x, y = map(int, [point[0] * width, point[1] * height])
            x = min(x, width - 1)
            y = min(y, height - 1)
            target_map[y, x] = 1  # 标记正样本

    #added to check if more than one class in target map 
    if np.unique(target_map).size < 2:
        print("⚠️ Warning: Only one class in target map, skipping this sample.")
        return float('nan')

    resized_heatmap = torch.nn.functional.interpolate(
        heatmap.unsqueeze(0).unsqueeze(0), size=(height, width), mode='bilinear', align_corners=False
    ).squeeze()

    auc = roc_auc_score(target_map.flatten(), resized_heatmap.cpu().flatten().numpy())
    return auc
